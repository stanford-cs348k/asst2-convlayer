...
This code will run your version of the convolution layer using randomly generated inputs and weights. It will run for three trials, and report the minimum time of the three runs. To run correctly you must ensure that Halide is in your library load path. On OSX this can be done like so:
    ...
    DYLD_LIBRARY_PATH=<my_halide_dir>/lib ./bin/convlayer <args>

and in the bash shell on Linux it will be:

    LD_LIBRARY_PATH=<my_halide_dir>/lib ./bin/convlayer <args>

and in the tcsh shell on Linux it will be:

    env LD_LIBRARY_PATH=<my_halide_dir>/lib ./bin/convlayer <args>
...
```
(base) student@host ~/cs348k/asst2-convlayer $ LD_LIBRARY_PATH=<my_halide_dir>/lib ./bin/convlayer --schedule auto
Schedule     : auto
width        : 128
height       : 128
channels     : 3
...
(base) student@host ~/cs348k/asst2-convlayer$ LD_LIBRARY_PATH=<my_halide_dir>/lib ./bin/convlayer --schedule student
Schedule     : student
width        : 128
height       : 128
channels     : 3
